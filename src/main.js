...
(function (ptype) {
    ptype["Circle"] = "circle";
    ptype["Square"] = "square";
    ptype["Rectangle"] = "rect";
    ptype["Path"] = "path";
})(ptype || (ptype = {}));
var btype;
(function (btype) {
    btype["Spawn"] = "spawn";
    btype["Station"] = "station";
})(btype || (btype = {}));
var rtype;
(function (rtype) {
    rtype["Expressway"] = "expwy";
    rtype["Railway"] = "railway";
    rtype["Route"] = "route";
})(rtype || (rtype = {}));
class stat {
    static zoom = 1;
...
class util {
    ...
    static async getdb(s) {
        let res = fetch(`./db/${s}.json`);
        return res.then(res => res.json());
    }
}
class draw {
    static LOW_newimg = (s, ctx, x, y, w, h) => { let tmp = new Image(); tmp.src = s; tmp.onload = () => { ctx.drawImage(tmp, x, y, w, h); }; };
    static LOW_xposcalc = (x, canvas) => (canvas.width) / 2 + (x * stat.zoom) - (stat.x * stat.zoom);
    static LOW_yposcalc = (y, canvas) => (canvas.height) / 2 + (y * stat.zoom) - (stat.y * stat.zoom);
    static cvClear = (ctx, canvas) => ctx.clearRect(0, 0, canvas.width, canvas.height);
    static cvPin = (ctx, canvas, x, y, icon, color, text) => {
        let cx, cy;
    ...
    static cvText = (ctx, font, x, y, s) => {
        ctx.font = font;
        ctx.fillText(s, x, y);
    };
    static cvCircleS = (ctx, cx, cy, long, color, weight) => {
        ctx.strokeStyle = color;
    ...
    static cvRectS = (ctx, cx, cy, long, long2, color, weight) => {
        ctx.strokeStyle = color;
        ctx.lineWidth = weight;
    ...
    // Wrapper
    static cvSquareS = (ctx, cx, cy, long, color, weight) => this.cvRectS(ctx, cx, cy, long, long, color, weight);
    static cvPath = (ctx, canvas, fx, fy, tx, ty, color, weight) => {
        ctx.strokeStyle = color;
        ctx.lineWidth = weight;
        ctx.beginPath();
